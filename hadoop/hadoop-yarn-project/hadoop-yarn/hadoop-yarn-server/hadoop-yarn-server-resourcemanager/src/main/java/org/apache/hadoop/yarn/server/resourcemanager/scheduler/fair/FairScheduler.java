...
/**
 * A scheduler that schedules resources between a set of queues. The scheduler keeps track of the
 * resources used by each queue, and attempts to maintain fairness by scheduling tasks at queues
 * whose allocations are farthest below an ideal fair distribution.
 * 
 * The fair scheduler supports hierarchical queues. All queues descend from a queue named "root".
 * Available resources are distributed among the children of the root queue in the typical fair
 * scheduling fashion. Then, the children distribute the resources assigned to them to their
 * children in the same fashion. Applications may only be scheduled on leaf queues. Queues can be
 * specified as children of other queues by placing them as sub-elements of their parents in the
 * fair scheduler configuration file.
 * 
 * A queue's name starts with the names of its parents, with periods as separators. So a queue named
 * "queue1" under the root named, would be referred to as "root.queue1", and a queue named "queue2"
 * under a queue named "parent1" would be referred to as "root.parent1.queue2".
 */
@LimitedPrivate("yarn")
@Unstable
...
public class FairScheduler extends AbstractYarnScheduler<FSAppAttempt, FSSchedulerNode> {
  ...
  private void validateConf(Configuration conf) {
    ...
    if (minMem < 0 || minMem > maxMem) {
      throw new YarnRuntimeException("Invalid resource scheduler memory" + " allocation configuration" + ", "
          ...
          + YarnConfiguration.RM_SCHEDULER_MAXIMUM_ALLOCATION_MB + "=" + maxMem + ", min should equal greater than 0"
  ...
  /**
   * Thread which calls {@link FairScheduler#update()} every <code>updateInterval</code>
   * milliseconds.
   */
   ...
   * deficits, minimum slot allocations, and amount of used and required resources per job.
   */
  protected synchronized void update() {
    ...
    if (LOG.isDebugEnabled()) {
      if (--updatesToSkipForDebug < 0) {
        ...
        LOG.debug("Cluster Capacity: " + clusterResource + "  Allocations: " + rootMetrics.getAllocatedResources()
            + "  Availability: "
            + Resource.newInstance(rootMetrics.getAvailableMB(), rootMetrics.getAvailableVirtualCores()) + "  Demand: "
            + rootQueue.getDemand());
      }
    }
  ...
  /**
   * Update the preemption fields for all QueueScheduables, i.e. the times since each queue last was
   * at its guaranteed share and over its fair share threshold for each type of task.
   */
  ...
  /**
   * Check for queues that need tasks preempted, either because they have been below their
   * guaranteed share for minSharePreemptionTimeout or they have been below their fair share
   * threshold for the fairSharePreemptionTimeout. If such queues exist, compute how many tasks of
   * each type need to be preempted and then select the right ones using preemptTasks.
   */
  ...
  /**
   * Preempt a quantity of resources. Each round, we start from the root queue, level-by-level,
   * until choosing a candidate application. The policy for prioritizing preemption for each queue
   * depends on its SchedulingPolicy: (1) fairshare/DRF, choose the ChildSchedulable that is most
   * over its fair share; (2) FIFO, choose the childSchedulable that is latest launched. Inside each
   * application, we further prioritize preemption by choosing containers with lowest priority to
   * preempt. We make sure that no queue is placed below its fair share in the process.
   */
  ...
  protected void warnOrKillContainer(RMContainer container) {
    ...
    if (time != null) {
      ...
      if (time + waitTimeBeforeKill < getClock().getTime()) {
        ...
        LOG.info("Killing container" + container + " (after waiting for premption for " + (getClock().getTime() - time)
            + "ms)");
      }
    } else {
  ...
  /**
   * Return the resource amount that this queue is allowed to preempt, if any. If the queue has been
   * below its min share for at least its preemption timeout, it should preempt the difference
   * between its current share and this min share. If it has been below its fair share preemption
   * threshold for at least the fairSharePreemptionTimeout, it should preempt enough tasks to get up
   * to its full fair share. If both conditions hold, we preempt the max of the two amounts (this
   * shouldn't happen unless someone sets the timeouts to be identical for some reason).
   */
  protected Resource resToPreempt(FSLeafQueue sched, long curTime) {
    ...
    if (Resources.greaterThan(RESOURCE_CALCULATOR, clusterResource, resToPreempt, Resources.none())) {
      String message = "Should preempt " + resToPreempt + " res for queue " + sched.getName() + ": resDueToMinShare = "
          + resDueToMinShare + ", resDueToFairShare = " + resDueToFairShare;
      LOG.info(message);
    }
    return resToPreempt;
  }
  ...
  /**
   * Add a new application to the scheduler, with a given id, queue name, and user. This will accept
   * a new app even if the user or queue is above configured limits, but the app will not be marked
   * as runnable.
   */
  protected synchronized void addApplication(ApplicationId applicationId, String queueName, String user,
    ...
    if (queueName.startsWith(".") || queueName.endsWith(".")) {
      String message = "Reject application " + applicationId + " submitted by user " + user
          + " with an illegal queue name " + queueName + ". " + "The queue name cannot start/end with period.";
      LOG.info(message);
      rmContext.getDispatcher().getEventHandler()
          .handle(new RMAppEvent(applicationId, RMAppEventType.APP_REJECTED, message));
      return;
  ...
  /**
   * Helper method that attempts to assign the app to a queue. The method is responsible to call the
   * appropriate event-handler if the app is rejected.
   */
  @VisibleForTesting
  ...
  private synchronized void removeApplicationAttempt(ApplicationAttemptId applicationAttemptId,
    ...
    // if(allocConf.getSchedulingPolicy(queue.getName()).getName().equalsIgnoreCase(InstantaneousGuaranteePolicy.NAME)){
     if (queue.getNumActiveApps()==0){ // TODO need to consider the case of the apps running not being active
       LOG.info("set queue: " + queue + " isRunning=false if there are no runnable apps");
  ...
  protected synchronized void completedContainer(RMContainer rmContainer, ContainerStatus containerStatus,
    ...
    if (application == null) {
      LOG.info("Container " + container + " of" + " unknown application attempt " + appId + " completed with event "
          + event);
      return;
    }
  ...
  private synchronized void removeNode(RMNode rmNode) {
    ...
    for (RMContainer container : runningContainers) {
      completedContainer(container,
          SchedulerUtils.createAbnormalContainerStatus(container.getContainerId(), SchedulerUtils.LOST_CONTAINER),
          RMContainerEventType.KILL);
    }
  ...
  public Allocation allocate(ApplicationAttemptId appAttemptId, List<ResourceRequest> ask, List<ContainerId> release,
    ...
    LOG.info(
        "allocate(" + appAttemptId + " " + ask + " " + release + " " + blacklistAdditions + " " + blacklistRemovals);
    ...
    if (ENABLE_ADMISSION) {
      if (queueMgr.getRootQueue().getPolicy().getName().equalsIgnoreCase(SpeedFairPolicy.NAME)) {
...
//          if (!queue.getQueueName().startsWith("interactive")) {
            LOG.info("[admission control] Cannot allocate " + appAttemptId + " because there is NO flexible resource left");
            return null;
          }
        }
    ...
    synchronized (application) {
      ...
      return new Allocation(allocation.getContainerList(), headroom, preemptionContainerIds, null, null,
  ...
  synchronized void attemptScheduling(FSSchedulerNode node) {
    ...
    if (reservedAppSchedulable != null) {
      ...
      if (!reservedAppSchedulable.hasContainerForNode(reservedPriority, node)
          || !fitsInMaxShare(queue, node.getReservedContainer().getReservedResource())) {
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug("Trying to fulfill reservation for application " + reservedAppSchedulable.getApplicationAttemptId()
              + " on node: " + node);
        }
  ...
  static boolean fitsInMaxShare(FSQueue queue, Resource additionalResource) {
    Resource usagePlusAddition = Resources.add(queue.getResourceUsage(), additionalResource);
  ...
  /**
   * Subqueue metrics might be a little out of date because fair shares are recalculated at the
   * update interval, but the root queue metrics needs to be updated synchronously with allocations
   * and completions so that cluster metrics will be consistent.
   */
  ...
  /**
   * Check if preemption is enabled and the utilization threshold for preemption is met.
   *
  ...
  public void handle(SchedulerEvent event) {
    ...
    case APP_ATTEMPT_ADDED:
      ...
      addApplicationAttempt(appAttemptAddedEvent.getApplicationAttemptId(),
          appAttemptAddedEvent.getTransferStateFromPreviousAttempt(), appAttemptAddedEvent.getIsAttemptRecovering());
      break;
    case APP_ATTEMPT_REMOVED:
      ...
      removeApplicationAttempt(appAttemptRemovedEvent.getApplicationAttemptID(),
          appAttemptRemovedEvent.getFinalAttemptState(), appAttemptRemovedEvent.getKeepContainersAcrossAppAttempts());
      break;
  ...
  /**
   * Helper for moveApplication, which has appropriate synchronization, so all operations will be
   * atomic.
   */
   ...
   * @throws YarnException
   *           if queue to remove is either not a leaf or if its not empty
   */
  @Override
