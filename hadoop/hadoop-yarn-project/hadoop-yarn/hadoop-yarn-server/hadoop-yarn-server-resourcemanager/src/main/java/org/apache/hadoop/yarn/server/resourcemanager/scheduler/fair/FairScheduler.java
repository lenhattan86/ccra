...
public class FairScheduler extends
	...
	protected synchronized void update() {
		...
		if (LOG.isDebugEnabled()) {
			if (--updatesToSkipForDebug < 0) {
				...
				LOG.debug("Cluster Capacity: "
				    + clusterResource
				    + "  Allocations: "
				    + rootMetrics.getAllocatedResources()
				    + "  Availability: "
				    + Resource.newInstance(rootMetrics.getAvailableMB(),
				        rootMetrics.getAvailableVirtualCores()) + "  Demand: "
				    + rootQueue.getDemand());
			}
		}
	 ...
	 * level-by-level, until choosing a candidate application. The policy for
	 * prioritizing preemption for each queue depends on its SchedulingPolicy: (1)
	 * fairshare/DRF, choose the ChildSchedulable that is most over its fair
	 * share; (2) FIFO, choose the childSchedulable that is latest launched.
	 * Inside each application, we further prioritize preemption by choosing
	 * containers with lowest priority to preempt. We make sure that no queue is
	 * placed below its fair share in the process.
	 */
	protected void preemptResources(Resource toPreempt) {
		...
		while (warnedIter.hasNext()) {
			...
			if ((container.getState() == RMContainerState.RUNNING || container
			    .getState() == RMContainerState.ALLOCATED)
...
//    }
	...
	protected synchronized void completedContainer(RMContainer rmContainer,
		...
		if (application == null) {
			LOG.info("Container " + container + " of"
			    + " unknown application attempt " + appId + " completed with event "
			    + event);
			return;
		}
	...
	public Allocation allocate(ApplicationAttemptId appAttemptId,
		...
		LOG.info("allocate(" + appAttemptId + " " + ask + " " + release + " "
		...
		// iglf - START
		if(allocConf.getSchedulingPolicy(application.getQueueName()).getName().equalsIgnoreCase(InstantaneousGuaranteePolicy.NAME)){
			// if the remaining flexible is less than the demand
			if (application.getFlexibleResource().isEmpty()) {
				if (!application.getQueue().isRunning()) { // this cause the accepted app can no longer get resource
					LOG.info("Cannot allocate " + appAttemptId
					    + " because there is NO flexible resource left");
					return null;
				}
			}
	
			LOG.info("allocate resource to application attempt: " + appAttemptId
			    + " queue:" + application.getQueue().getQueueName()
			    + " isRunning=true ");
			application.getQueue().setIsRunning(true);
		}
		// iglf - END
		...
		synchronized (application) {
			if (!ask.isEmpty()) {
				if (LOG.isDebugEnabled()) {
					LOG.debug("allocate: pre-update" + " applicationAttemptId="
					    + appAttemptId + " application=" + application.getApplicationId());
				}
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug("allocate: post-update" + " applicationAttemptId="
				    + appAttemptId + " #ask=" + ask.size() + " reservation= "
	...
	private class NodeAvailableResourceComparator implements Comparator<NodeId> {
		...
		public int compare(NodeId n1, NodeId n2) {
			...
			return RESOURCE_CALCULATOR.compare(clusterResource, nodes.get(n2)
			    .getAvailableResource(), nodes.get(n1).getAvailableResource());
		}
	}
	...
	synchronized void attemptScheduling(FSSchedulerNode node) {
		...
		if (reservedAppSchedulable != null) {
			...
			if (!reservedAppSchedulable.hasContainerForNode(reservedPriority, node)
			    || !fitsInMaxShare(queue, node.getReservedContainer()
			        .getReservedResource())) {
				...
				if (LOG.isDebugEnabled()) {
					LOG.debug("Trying to fulfill reservation for application "
					    + reservedAppSchedulable.getApplicationAttemptId() + " on node: "
					    + node);
				}
		...
		if (reservedAppSchedulable == null) {
			...
			while (node.getReservedContainer() == null) {
				...
				if (!queueMgr.getRootQueue().assignContainer(node)
	...
	static boolean fitsInMaxShare(FSQueue queue, Resource additionalResource) {
		Resource usagePlusAddition = Resources.add(queue.getResourceUsage(),
		    additionalResource);
	...
	/**
	 * Check if preemption is enabled and the utilization threshold for preemption
	 * is met.
	 *
		...
		case CONTAINER_EXPIRED:
			...
			completedContainer(getRMContainer(containerId),
			    SchedulerUtils.createAbnormalContainerStatus(containerId,
			        SchedulerUtils.EXPIRED_CONTAINER), RMContainerEventType.EXPIRE);
			break;
	...
	private synchronized String resolveReservationQueueName(String queueName,
		...
		if (reservationID != null) {
			...
			if (!queue.getParent().getQueueName().equals(queueName)) {
				String message = "Application: " + applicationId
				    + " submitted to a reservation " + resQName
	...
	private void initScheduler(Configuration conf) throws IOException {
		synchronized (this) {
			...
			preemptionUtilizationThreshold = this.conf
	...
	@Override
	public void setEntitlement(String queueName, QueueEntitlement entitlement)
