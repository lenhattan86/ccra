...
public class FairScheduler
  ...
  protected synchronized void update() {
    ...
    // Recursively compute fair shares for all queues and update metrics
  ...
  protected synchronized void addApplicationAttempt(
    ...
    if (isAttemptRecovering) {
      ...
      rmContext.getDispatcher().getEventHandler().handle(new RMAppAttemptEvent(
  ...
  FSLeafQueue assignToQueue(RMApp rmApp, String queueName, String user) {
    ...
    if (appRejectMsg != null && rmApp != null) {
      ...
      rmContext.getDispatcher().getEventHandler().handle(new RMAppEvent(
          rmApp.getApplicationId(), RMAppEventType.APP_REJECTED, appRejectMsg));
      return null;
    }
  ...
  private synchronized void removeApplicationAttempt(
    ...
    // Inform the queue
    FSLeafQueue queue = queueMgr.getLeafQueue(attempt.getQueue().getQueueName(),
        false);
  ...
  protected synchronized void completedContainer(RMContainer rmContainer,
    ...
    if (application == null) {
      LOG.info(
          "Container " + container + " of" + " unknown application attempt "
  ...
  private void updateRootQueueMetrics() {
    rootMetrics.setAvailableResourcesToQueue(Resources.subtract(clusterResource,
  ...
  private void verifyMoveDoesNotViolateConstraints(FSAppAttempt app,
    ...
    while (cur != lowestCommonAncestor) {
      // maxRunningApps
      if (cur.getNumRunnableApps() == allocConf
          .getQueueMaxApps(cur.getQueueName())) {
