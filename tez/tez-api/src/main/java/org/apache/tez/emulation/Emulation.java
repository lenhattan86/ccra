...
public class Emulation {
  ...
  public static byte[] createConfiguration(long taskDuration) throws IOException {
    ...
    if (fr.isDirectory() || !fr.exists()) {
      ...
      try {
        ...
        while ((line = br.readLine()) != null) {
          ...
          if (args.length <= 2)
            LOG.info("readWorkloadTraces: Incorrect node entry");
          ...
          for (int i = 0; i < numVertices; ++i) {
            ...
            // .setUserPayload(new
            // SleepProcessor.SleepProcessorConfig(100000).toUserPayload()), 1)
            ...
            dag.addVertex(v);
            vertices.add(v);
            // LOG.info("readWorldloadTrace: DAG:"+dag.getName() + "
            // addVertex:"+vertices.get(vertices.size()-1).getName());
          }
          ...
          for (int i = 0; i < numEdgesBtwStages; ++i) {
            ...
            // assert (args.length == 3) : "Incorrect entry for edge
            // description; [stage_src stage_dst comm_type]";
            ...
            // LOG.info("readWorldloadTrace: Edge:"+ srcVertex.getName() + " -->
            // " + dstVertex.getName());
  ...
  public DAG createDAGFromTrace(String dagName) throws Exception {
    DAG encodedDag = null;
    boolean isProfile = this.conf.getBoolean(TezConfiguration.TEZ_DAG_PROFILE_ENABLE,
        TezConfiguration.TEZ_DAG_PROFILE_ENABLE_DEFAULT);
    if (!isProfile) {
      List<DAG> dags = this.readWorkloadTraces();
      if (dags.isEmpty())
        return null;

      encodedDag = findDag(dags, dagName);
    } else {
      encodedDag = createDAGFromProfile(dagName);
    }

    return encodedDag;
  }

  private DAG createDAGFromProfile(String dagName) {
    String profilePath = TezConfiguration.TEZ_DAG_PROFILE_PATH_DEFAULT;
    DAG dag = DAG.create("" + dagName);

    profilePath = this.conf.get(TezConfiguration.TEZ_DAG_PROFILE_PATH, TezConfiguration.TEZ_DAG_PROFILE_PATH_DEFAULT);
    
    String file = profilePath + dagName + ".profile";
    ...
    if (fr.isDirectory() || !fr.exists()) {
      ...
      try {
        ...
        String[] args = line.split(" ");
        if (args.length <= 2)
          LOG.info("createDAGFromProfile: Incorrect node entry");
        ...
        for (int i = 0; i < numEdgesBtwStages; ++i) {
          args = br.readLine().split(" ");
          // assert (args.length == 3) : "Incorrect entry for edge description;
          // [stage_src stage_dst comm_type]";
          ...
          dag.addEdge(Edge.create(srcVertex, dstVertex,
              EdgeProperty.create(DataMovementType.SCATTER_GATHER, DataSourceType.PERSISTED, SchedulingType.SEQUENTIAL,
                  DumpOutput.getOutputDesc(payload), DumpInput.getInputDesc(payload))));
        }
        br.close();
      } catch (Exception e) {
        System.err.println("Catch exception: " + e);
  ...
  public static class InputInitializerForTest extends InputInitializer {
    ...
    public InputInitializerForTest(InputInitializerContext initializerContext) {
      ...
      getContext().registerForVertexStateUpdates(EVENT_GENERATING_VERTEX_NAME, EnumSet.of(VertexState.SUCCEEDED));
    }
    ...
    public void handleInputInitializerEvent(List<InputInitializerEvent> events) throws Exception {
      ...
      try {
        for (InputInitializerEvent event : events) {
          Preconditions.checkArgument(event.getSourceVertexName().equals(EVENT_GENERATING_VERTEX_NAME));
    ...
    public void onVertexStateUpdated(VertexStateUpdate stateUpdate) {
      ...
      try {
        ...
        if (eventsSeen.cardinality() == getContext().getVertexNumTasks(EVENT_GENERATING_VERTEX_NAME)) {
          condition.signal();
        } else {
