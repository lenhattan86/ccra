 ...
 */
public class SimpleProcessor extends AbstractLogicalIOProcessor {
  private static final Logger LOG = LoggerFactory
      .getLogger(SimpleProcessor.class);
  ...
  public static ProcessorDescriptor getProcDesc(UserPayload payload) {
    return ProcessorDescriptor.create(SimpleProcessor.class.getName()).setUserPayload(
...
/*    if (!doRandomFail) {
    ...
    for (Map.Entry<String, LogicalOutput> entry : outputs.entrySet()) {
      if (!(entry.getValue() instanceof DumpOutput)) {
      ...
      DumpOutput output = (DumpOutput) entry.getValue();
      output.write(sum);
    }
