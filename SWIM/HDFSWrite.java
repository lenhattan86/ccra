...
//      numBytesToWrite = job.getLong("test.randomwrite.bytes_per_map", 128*1024*1024);
      ...
      System.out.println("[debug] keySizeRange: "+keySizeRange);
//      keySizeRange = job.getInt("test.randomwrite.max_key", 1000*1000) - minKeySize; // randomize the input sizes
      minValueSize = job.getInt("test.randomwrite.min_value", 0);
      System.out.println("[debug] minValueSize: "+minValueSize);
      valueSizeRange = 
  ...
  public int run(String[] args) throws Exception {    
    ...
    // must be less than or equal to the max Map Task per host (10).
    int numMapsPerHost = job.getInt("test.randomwriter.maps_per_host", 10); 
//    long numBytesToWritePerMap = job.getLong("test.randomwrite.bytes_per_map",  1*1024*1024*1024);
    long numBytesToWritePerMap = job.getLong("test.randomwrite.bytes_per_map",  67108864);
