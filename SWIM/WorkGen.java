...
public class WorkGen extends Configured implements Tool {
  ...
  public static double nLoop = 1*Math.pow(10, 5);
  
  static int printUsage() {
    System.out.println("sort [-m <maps>] [-r <reduces> has no effect] " + "[-inFormat <input format class>] "
   ...
   * Comments
   */
  static class RatioMapper extends MapReduceBase
    ...
    public void map(WritableComparable key, Writable val, OutputCollector<BytesWritable, BytesWritable> output,
...
//      WorkGen.runDumpCompuation(WorkGen.nLoop);
   ...
   * Comments
   */
  static class RatioReducer extends MapReduceBase
      implements Reducer<WritableComparable, Writable, BytesWritable, BytesWritable> {
...
//    int num_maps = (int) (cluster.getMaxMapTasks() * 0.9);
    ...
    for (int i = 0; i < args.length; ++i) {
      try {
        if ("-m".equals(args[i])) {
          ...
          queueName = args[++i];
        } else if ("-map.vcores".equals(args[i])) {
          mapVcores = Integer.parseInt(args[++i]);
        } else if ("-red.vcores".equals(args[i])) {
          redVcores = Integer.parseInt(args[++i]);
        } else if ("-map.memory".equals(args[i])) {
          mapMem = Integer.parseInt(args[++i]);
        } else if ("-red.memory".equals(args[i])) {
          redMem = Integer.parseInt(args[++i]);
        } else {
    ...
    jobConf.setMemoryForReduceTask(redMem);
    jobConf.setMemoryForMapTask(mapMem);
    
    jobConf.setLong(MRJobConfig.MAP_CPU_VCORES, mapVcores);
    jobConf.setLong(MRJobConfig.REDUCE_CPU_VCORES, redVcores);
...
//    }
    ...
    jobConf.set("mapreduce.job.queuename", queueName);
    ...
    System.out.println(
        "shuffleInputRatio  = " + Double.parseDouble(jobConf.getRaw("workGen.ratios.shuffleInputRatio")));
    System.out.println(
        "outputShuffleRatio = " + Double.parseDouble(jobConf.getRaw("workGen.ratios.outputShuffleRatio")));

    System.out.println("Running on " + cluster.getTaskTrackers() + " nodes with " + num_maps + " maps and "
        + num_reduces + " reduces.");
    ...
    FileWriter timeLog = new FileWriter("./"+queueName+".csv", true);
    String toWrite = otherArgs.get(0) + ", " + startTime + ", " + (end_time.getTime() - startTime.getTime()) / 1000
        + ", " + end_time + ", " + queueName + "\n";
