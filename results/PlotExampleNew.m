...
%   method = 'DRF';  
 method = 'SP';  
%   method = 'SimpleBestEffort';  
%   method = 'SimpleBestRejectAll';  
...
%%
if plots(1) 
  START_TIME = 1; END_TIME = 2000+START_TIME;  
  lengendStr = {'LQ A', 'LQ B', 'LQ C', 'TQ D'};
  ...
  if strcmp(method,'BPF')          
     ...
     % LQ 2
     stage2Period = LQ1Demands(2)*period;
     for j = 1:jobNum
       startTime = start_time_step + period*(j-1)+queueUpPeriod+stage1Period;
       usedMEM(2, startTime:stage2Period+startTime) = MAX_MEM/GB;
     end   
     
     % LQ 3     
     stage3Period = LQ2Demands(2)*period;
     for j = 1:jobNum
       startTime = start_time_step + period*(j-1)+queueUpPeriod;
       usedMEM(3, startTime:stage1Period+startTime) = MAX_MEM/GB/2;  
       usedMEM(3, stage1Period+startTime+1:period+startTime) = MAX_MEM/GB/2;
     end   
     
     % TQ 0
     usedMEM(4, :) = MAX_MEM/GB;
     
   elseif strcmp(method,'DRF')
     
     % LQ 1     
     stage1Period = LQ1Demands(2)*period*num_queues;
     for j = 1:jobNum
       startTime = start_time_step + period*(j-1)+queueUpPeriod;
       usedMEM(1, startTime:stage1Period+startTime) = MAX_MEM/GB/4;
     end   
     
     % LQ 2     
     stage2Period = LQ2Demands(2)*period*num_queues;
     for j = 1:jobNum
       startTime = start_time_step + period*(j-1)+queueUpPeriod;
       usedMEM(2, startTime:stage2Period+startTime) = MAX_MEM/GB/4;
     end   
     
     % LQ 3     
     for j = 1:jobNum
       startTime = start_time_step + period*(j-1)+queueUpPeriod;
       usedMEM(3, startTime:stage1Period+startTime) = MAX_MEM/GB/4;       
       usedMEM(3, stage1Period+startTime+1:period+startTime) = MAX_MEM/GB/2;
     end   
   ...
   elseif strcmp(method,'SP')       
     ...
     % LQ 1     
     stage1Period = LQ1Demands(2)*period*(num_queues-1);
     for j = 1:jobNum
       startTime = start_time_step + period*(j-1)+queueUpPeriod;
       usedMEM(1, startTime:stage1Period+startTime) = MAX_MEM/GB/(num_queues-1);
     end   
     
     % LQ 2     
     stage2Period = LQ2Demands(2)*period*(num_queues-1);
     for j = 1:jobNum
       startTime = start_time_step + period*(j-1)+queueUpPeriod;
       usedMEM(2, startTime:stage2Period+startTime) = MAX_MEM/GB/(num_queues-1);
     end   
     
     % LQ 3     
     stage3Period = LQ2Demands(2)*period*(num_queues-1);
     for j = 1:jobNum
       startTime = start_time_step + period*(j-1)+queueUpPeriod;
       usedMEM(3, startTime:stage3Period+startTime) = MAX_MEM/GB/(num_queues-1);  
       usedMEM(3, stage3Period+startTime+1:period+startTime) = MAX_MEM/GB;
     end   
   ...
   elseif strcmp(method,'SimpleBestEffort')       
     ...
     for j = 1:jobNum
       startTime = start_time_step + period*(j-1)+queueUpPeriod;
       usedMEM(3, startTime:stage1Period+startTime) = MAX_MEM/GB/2;  
       usedMEM(3, stage1Period+startTime+1:period+startTime) = MAX_MEM/GB;
     end   
     
     % TQ 0
     usedMEM(4, :) = MAX_MEM/GB;
   elseif strcmp(method,'SimpleBestRejectAll')       
     ...
     % LQ 2
     stage2Period = LQ1Demands(2)*period*(num_queues-1);
     for j = 1:jobNum
       startTime = start_time_step + period*(j-1)+queueUpPeriod+ stage1Period;
       usedMEM(2, startTime:stage2Period+startTime) = MAX_MEM/GB/(num_queues-1);
     end   
     ...
     % LQ 3     
%     stage3Period = LQ2Demands(2)*period*(num_queues-2);
     for j = 1:jobNum
       startTime = start_time_step + period*(j-1)+queueUpPeriod + stage1Period;
       usedMEM(3, startTime:stage2Period+startTime) = MAX_MEM/GB/(num_queues-1);  
       endIdx = min(period+startTime, length(usedMEM(1,:)));
       usedMEM(3, startTime+stage2Period+1:endIdx) = MAX_MEM/GB/(num_queues-2);
     end   
...
%       subplot(2,1,2); 
      ...
      if is_printed   
        figIdx=figIdx +1;
        fileNames{figIdx} = ['b' int2str(num_interactive_queue) '_example_' method];        
